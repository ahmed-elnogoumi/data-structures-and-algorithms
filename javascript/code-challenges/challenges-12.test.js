...
const count = (target, input) => {
  return input.reduce((acc, curr) => acc + curr.filter(num => num === target).length, 0);
};
...
const totalSum = (input) => {
  return input.flat().reduce((acc, num) => acc + num, 0);
};
...
let findShortest = (data) => {
  return data.reduce((acc, cur) => parseInt(acc.height) < parseInt(cur.height) ? acc : cur).name;
};
